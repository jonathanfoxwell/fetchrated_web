...
export const updateArticleSchema = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      cta_type: { type: 'string', enum: ['find-location', 'join-pilot', 'get-verified', 'custom'] },
...
export async function updateArticle(params: Record<string, unknown>): Promise<{ content: { type: 'text'; text: string }[] }> {
  ...
  if (updates.sections && Array.isArray(updates.sections)) {
    for (let i = 0; i < updates.sections.length; i++) {
      const error = validateSection(updates.sections[i] as Record<string, unknown>, i);
      if (error) {
        return {
          content: [{
            type: 'text',
  ...
  if (updates.excerpt) {
    const excerpt = updates.excerpt as string;
...
export async function getArticle(params: Record<string, unknown>): Promise<{ content: { type: 'text'; text: string }[] }> {
  const { slug } = params;
