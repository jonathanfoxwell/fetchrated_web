...
export async function publishArticle(params: Record<string, unknown>): Promise<{ content: { type: 'text'; text: string }[] }> {
  const { slug } = params;
...
export async function unpublishArticle(params: Record<string, unknown>): Promise<{ content: { type: 'text'; text: string }[] }> {
  const { slug } = params;
